...
{
    /// <summary>
    /// Summary description for CustomFieldChange
    /// </summary>
    public class CustomFieldChange : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var customFields = Queries.GetDistinctCountriesByCustomField(moduleId, maxPoints, customField);
            var customFieldMarkers = Queries.GetMarkersByCustomField(moduleId, customField, country, state, city, maxPoints);

            var countriesHtml = SharedMethods.CreateCountryHtml(customFields);
            var states = Queries.GetDistinctStates(moduleId, customField, country, maxPoints);
            var statesHtml = SharedMethods.CreateStatesHtml(customFields);
            ...
            var markerCollection = Queries.GetMarkersByCustomField(moduleId, customField, country, state, city, maxPoints);

            var markers = new List<ViewAbleMarker>();
            markerCollection.ToList().ForEach(marker => markers.Add(new ViewAbleMarker(marker)));

            var jsonObject = new { countries = countriesHtml, states = statesHtml, cities = citiesHtml, message = String.Empty, success = true, markers };
